...
class FixedFourierFeatureEncoding(nn.Module):
    def __init__(self, input_dim, num_frequencies=10, include_input=False):
        ...
        self.register_buffer("freq_bands", 2 ** torch.arange(num_frequencies).float())  # [num_frequencies]
    ...
    def forward(self, x):
        ...
        encoded = encoded.view(x.shape[0], -1)  # [batch, input_dim * 2 * num_frequencies]

        if self.include_input:
            return torch.cat([x.squeeze(-1), encoded], dim=-1)  # [batch, input_dim + encoded_dim]
        else:
            return encoded
...
class RandomFourierFeatureEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        features = torch.cat([torch.sin(x_proj), torch.cos(x_proj)], dim=-1)  # [batch, 2*num_features]

        if self.include_input:
            return torch.cat([x, features], dim=-1)  # [batch, input_dim + 2*num_features]
        else:
            return features
...
class Model(nn.Module):
    def __init__(
        ...
        if not distance_transform_off:
            ...
            self.origins = nn.Parameter(torch.tensor([[5.0,2,5],[5,4,5],[5,6,5],[5,5,4]]))
        ...
        if rff_off:
            ...
            self.fourier = FixedFourierFeatureEncoding(input_dim=3, num_frequencies=100, include_input=True)
            ff_dim = 3 * 2 * 100 + num_features
            self.network = nn.Sequential(
                nn.Linear(ff_dim, 1024),
                nn.LeakyReLU(),
                *[nn.Sequential(nn.Linear(1024, 1024), nn.LeakyReLU()) for _ in range(1)],
                nn.Linear(1024, num_classes)
            )
    ...
    def forward(self, xyz):
        ...
        if not self.memory_off:
            distances = torch.cat([self.memory.expand(xyz.shape[0], -1), distances], dim=-1)
        ...
        if not self.attention_off:
            ...
            attn_scores = torch.matmul(Q, K.transpose(-2, -1)) / (self.attn_dim ** 0.5)  # [B, 1, T]
...
def zero_grad_on_correct(outputs, targets):
    ...
    correct_mask = (preds != targets).float()      # [B] — 1 for incorrect predictions, 0 for correct
...
class CelltypeModel(nn.Module):
    def __init__(self, slices, n_classes, val_slice=None, learning_rate=0.001, batch_size=32, epochs=10, device=None):
