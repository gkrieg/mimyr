...
def parallel_sparse_matrix(
    result_data_values, result_data_rows, result_data_cols, n_row, n_col, num_threads=4
):
...
def GeneSymbolUniform(
    ...
    ref_table_raw = ref_table_raw[
        ["Approved symbol", "Alias symbol", "Previous symbol"]
    ]

    # Filter rows with non-empty "Previous symbol" and "Alias symbol"
    ref_table_raw = ref_table_raw[
        (
            ref_table_raw["Previous symbol"].notna() & ref_table_raw["Previous symbol"]
            != ""
        )
    ...
    # Replace "_" with "-"
    ref_table_raw["Previous symbol"] = ref_table_raw["Previous symbol"].str.replace(
        "_", "-"
    )
    ref_table_raw["Alias symbol"] = ref_table_raw["Alias symbol"].str.replace("_", "-")
    ref_table_raw["Approved symbol"] = ref_table_raw["Approved symbol"].str.replace(
        "_", "-"
    )

    # Create subsets for Previous and Alias symbols
    ref_table_prev = ref_table_raw[
        ["Approved symbol", "Previous symbol"]
    ].drop_duplicates()
    ref_table_prev = ref_table_prev[
        ref_table_prev["Previous symbol"].notna() & ref_table_prev["Previous symbol"]
        != ""
    ]

    ref_table_alia = ref_table_raw[
        ["Approved symbol", "Alias symbol"]
    ].drop_duplicates()
    ref_table_alia = ref_table_alia[
        ref_table_alia["Alias symbol"].notna() & ref_table_alia["Alias symbol"] != ""
    ]
    ...
    total_gene_list_raw = pd.read_csv(
        gene_list_path, sep="\t", header=0, na_values=pd.NA
    )
    ...
    for query_symbol in tqdm(query_gene_list, desc="Processing"):
        ...
        if query_symbol in total_gene_list:
            ...
            candidates_A = ref_table_alia.loc[
                ref_table_alia["Alias symbol"] == query_symbol, "Approved symbol"
            ].values
            candidates_P = ref_table_prev.loc[
                ref_table_prev["Previous symbol"] == query_symbol, "Approved symbol"
            ].values
    ...
    gene_appearance_dict_filtered = {
        key: value for key, value in gene_appearance_dict.items() if len(value) > 0
    }
    ...
    total_gene_list_indices = {
        gene: index for index, gene in enumerate(total_gene_list)
    }
    query_gene_list_indices = {
        gene: index for index, gene in enumerate(query_gene_list)
    }
    ...
    for key, source_genes in tqdm(
        gene_appearance_dict_filtered.items(), desc="Processing"
    ):
        target_col = total_gene_list_indices.get(key)
        source_col = np.unique([query_gene_list_indices[gene] for gene in source_genes])

        col_sum = (
            query_data_csc[:, source_col].sum(axis=1).A.flatten()
        )  # Sum the values
    ...
    if n_row >= 1e5:
        logger.info(
            f"Because the number of cells is very large, we construct the sparse matrix parallelly with {n_threads} threads"
        )
        result_data = parallel_sparse_matrix(
            result_data_values,
            result_data_rows,
            result_data_cols,
            n_row,
        ...
        result_data = sp.csr_matrix(
            (result_data_values, (result_data_rows, result_data_cols)),
            shape=(n_row, n_col),
        )
    logger.info(
        f"Shape of output data is {result_data.shape}. It should have 42117 genes with cell number unchanged."
    )

    # *--------------------Build output data--------------------*
    adata_output = sc.AnnData(
        X=result_data, obs=adata_input.obs, var=pd.DataFrame(index=total_gene_list)
    )

    adata_output.write(
        os.path.join(output_dir, "{}_uniformed.h5ad".format(output_prefix))
    )
    logger.info(
        f"h5ad file saved in:{os.path.join(output_dir,f'{output_prefix}_uniformed.h5ad')}"
    )

    if print_report:
        report.to_csv(os.path.join(output_dir, "{}_report.csv".format(output_prefix)))
        logger.info(
            f"report file saved in: {os.path.join(output_dir,f'{output_prefix}_report.csv')}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_path", type=str, help="the path of the input h5ad file"
    )
    parser.add_argument(
        "--ref_table_path",
        default="./GeneSymbolRef_upd0731.csv",
        type=str,
        help="the path of the reference table.",
    )
    parser.add_argument(
        "--gene_list_path",
        default="./total_gene_list_42117.txt",
        type=str,
        help="the path of the total gene list table.",
    )
    parser.add_argument(
        "--output_dir",
        default="./",
        type=str,
        help="the path to save the output h5ad file",
    )
    parser.add_argument(
        "--output_prefix",
        default="",
        type=str,
        help="the prefix of the output file and report file",
    )
    parser.add_argument(
        "--print_report",
        action="store_true",
        help="print a report of the modified genes in the report.csv under the output_dir",
    )
    parser.add_argument(
        "--average_alias",
        action="store_true",
        help="if average the counts of the genes mapped to the same aprroved symbol",
    )
    parser.add_argument(
        "--n_threads",
        default=1,
        type=int,
        help="number of threads used in the construction of sparse matrix",
    )
